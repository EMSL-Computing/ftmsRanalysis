...
test_that("mapCompoundsToModules works correctly mapping to KEGG", {
  ...
  suppressWarnings(compIcrData <- mapPeaksToCompounds(peakIcrProcessed, db="KEGG"))
  suppressWarnings(modIcrData <- mapCompoundsToModules(compIcrData))
...
test_that("mapCompoundsToModules fails correctly with bad parameters", {
  ...
  expect_error(tmp <- mapCompoundsToModules(peakIcrProcessed), regexp = "must be an object of type compoundIcrData")
  expect_error(tmp <- mapCompoundsToModules(iris), regexp = "must be an object of type compoundIcrData")
