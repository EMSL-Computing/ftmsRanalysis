...
one_sample <- subset(peakIcrProcessed, samples="EM0011_sample")
...
```{r}
densityPlot(peakIcrProcessed, variable="NOSC", samples="EM0069_sample")
```
...
```{r}
densityPlot(peakIcrProcessed, variable="NOSC", samples=c("EM0011_sample", "EM0013_sample", "EM0015_sample"),
            curve_colors=c("blue", "red", "green"))
```

Since the `peakIcrProcessed` object has a group definition (see `group_designation` for how to construct this), the `groups` parameter can be used to select multiple groups to plot (these curves are the distribution combining all peaks observed by samples in the group).

```{r}
#getGroupDF(peakIcrProcessed)
densityPlot(peakIcrProcessed, variable="NOSC", samples=FALSE, groups=c("M_C", "M_S"))
```
...
```{r}
MSgroup <- subset(peakIcrProcessed, groups="M_S")
