...
Van Krevelen (VK) plots do something useful that I will have to find out about some other time. 

A Van Krevelen plot displays the H:C versus O:C ratios for each peak observed. The `fticRanalysis` package 
...
Please note that the Rivas-Ubach (`bs3`) boundaries should only be used for plant matter data. __Lisa: do we want to insert more 
discussion on the differences between these boundary sets?__
...
```{r, message=FALSE}
ms_group <- subset(peakIcrProcessed, groups="M_S")
...
```{r}
m_groups <- divideByGroupComparisons(peakIcrProcessed, comparisons=list(c("M_S", "M_C")))[[1]]$value
