...
cmsDat_unique_mf <-  cmsDat %>% 
  applyFilt(filter_object = cmsDat_filt_obj,
                      msObj = .,
                      min_conf = 0.16) %>% 
  unique_mf_assingment(method = "confidence")
```


# Convert CoreMSData object into ftmsData object

Once unique molecular formulas have been assigned to peaks within each sample, we can convert the `CoreMSData` object into an `ftmsData` object using the function `CoreMSData_to_ftmsData`. This transforms the data table containing monoisotopic peaks into a list of three data tables:
...
For further steps and information, see [this introduction to `ftmsRanalysis`](ftmsRanalysis.html).
