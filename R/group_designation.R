...
group_designation <- function(icrData, main_effects, covariates=NULL, var.name=FALSE){
  ...
  if(!inherits(icrData, "icrData")) stop("icrData is not an object of appropriate class")
  ...
  if(sum(main_effects %in% names(icrData$f_data))!=length(main_effects)) stop("One or more of the main_effects is not found in f_data of icrData")
  
  ### end of preliminary checks ###
  
  # store f_data temporarily #
  temp_data = icrData$f_data
  
  # pull sample id column name #
  samp_id = attr(icrData, "cnames")$fdata_cname
  ...
  if(length(all.na) > 0){
    ...
    edat_ids = which(names(icrData$e_data) %in% all.na)
    fdat_ids = which(temp_data[ , samp_id] %in% all.na)
    
    icrData$e_data = icrData$e_data[,-edat_ids]
    icrData$f_data = icrData$f_data[-fdat_ids,]
  }else{output2 = output}
  
  attr(output2, "main_effects") = main_effects
  attr(output2, "covariates") = covariates
  attr(icrData, "group_DF") = output2
