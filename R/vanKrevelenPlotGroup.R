...
#' @export
vanKrevelenPlotGroup <- function(data.obj, title=NA, colors=NA, xlabel="O:C Ratio", ylabel="H:C Ratio") {
  require(dplyr)
  require(plotly)
  require(scales)
  # 
  ...
  OC.col <- icRanalysis:::getOCRatioColName(data.obj)
  ...
  HC.col <- icRanalysis:::getHCRatioColName(data.obj)
  ...
  emeta.df[,icRanalysis:::getEDataColName(data.obj)] = as.character(emeta.df[,icRanalysis:::getEDataColName(data.obj)])
  
  # Include only rows (peaks) where that are observed in at least one column of e_data
  emeta.df <- merge(emeta.df, data.obj$e_data, by=icRanalysis:::getEDataColName(data.obj))
  emeta.df <- emeta.df[which(rowSums(data.obj$e_data[,c(2:3)], na.rm=TRUE) > 0),]
  ...
  vankrev_categories <- icRanalysis:::getVanKrevelenPlottingDF()
  
  # color palette
  if (is.na(colors)) {
    col.pal<- scales::col_factor("Set1", levels=rownames(icRanalysis:::getVanKrevelenCategoryBounds()))   
    col.vec <- col.pal(rownames(icRanalysis:::getVanKrevelenCategoryBounds()))
    names(col.vec) <- rownames(icRanalysis:::getVanKrevelenCategoryBounds())
  ...
  hovertext <- emeta.df[, icRanalysis:::getMFColName(data.obj)]
  ...
  p <- ggplot(emeta.df, aes(x=emeta.df[,OC.col], y=emeta.df[,HC.col])) +
    geom_point(aes(fill=Pres),pch=21, size=4)+
